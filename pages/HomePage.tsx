...
import { FOUNDER_IMAGE_URL, eventsData, postsData, pledgeData, calculateReadingTime } from '../utils/data';
...
const SchemaMarkup = () => {
    const orgSchema = {
        ...
        "logo": "https://masaworld.org/logo.png",
        "sameAs": [
...
const HeroSlider: React.FC<NavigationProps> = ({ navigateTo }) => {
    ...
    return (
        <section className="relative bg-masa-charcoal text-white h-[90vh] min-h-[650px] flex items-center overflow-hidden">
            ...
            <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 z-10">
                <div className="max-w-4xl text-left">
                     ...
                     <h1 key={`head-${currentSlide}`} className="text-4xl md:text-6xl font-extrabold tracking-tight leading-tight animate-fade-in-up drop-shadow-lg">
                        {slides[currentSlide].headline}
                    </h1>
                    <p key={`sub-${currentSlide}`} className="mt-6 text-xl md:text-2xl text-gray-100 font-light max-w-3xl leading-relaxed animate-fade-in-up drop-shadow-md" style={{ animationDelay: '0.2s' }}>
                        {slides[currentSlide].subtext}
                    </p>
                    <div key={`cta-${currentSlide}`} className="mt-12 flex flex-col sm:flex-row gap-4 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                        {slides[currentSlide].ctas.map((cta, idx) => (
                            <button 
                                ...
                                className={`${cta.primary 
                                    ...
                                    px-8 py-4 rounded-full text-lg font-bold transition-all duration-300 transform hover:scale-105`}
                            >
                                {cta.label}
                            </button>
                        ))}
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex space-x-3">
                {slides.map((_, index) => (
                    <button 
                        ...
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-masa-orange w-10' : 'bg-white/50 hover:bg-white'}`}
...
// --- NGO HELP DESK TEASER ---
const NgoHelpDeskTeaser: React.FC<NavigationProps> = ({ navigateTo }) => (
    <section className="py-12 bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                <div className="text-center md:text-left">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-100 text-masa-blue text-xs font-bold mb-3">
                        <SparklesIcon className="h-3 w-3" />
                        <span>40+ AI Tools</span>
                    </div>
                    <h2 className="text-2xl font-bold text-masa-charcoal">NGO Help Desk</h2>
                    <p className="text-gray-600 mt-1">AI-powered tools to support NGOs, volunteers, and social initiatives.</p>
                </div>
                <button 
                    onClick={() => navigateTo('ngo-help-desk')} 
                    className="w-full md:w-auto bg-masa-charcoal text-white px-6 py-3 rounded-lg font-bold hover:bg-gray-800 transition-all shadow-md flex items-center justify-center gap-2"
                >
                    Open NGO Help Desk <ArrowRightIcon className="h-4 w-4" />
                </button>
            </div>
        </div>
    </section>
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="w-full md:w-1/2">
                        <h3 className="text-xl font-bold text-masa-charcoal">{featuredEvent.title}</h3>
                        <div className="flex items-center gap-4 text-sm text-gray-500 mt-2 mb-3">
                            <span className="flex items-center gap-1"><CalendarDaysIcon className="h-4 w-4 text-masa-blue"/> {featuredEvent.displayDate}</span>
                            <span className="flex items-center gap-1"><MapPinIcon className="h-4 w-4 text-masa-orange"/> {featuredEvent.location}</span>
                        </div>
                        <p className="text-gray-600 text-sm line-clamp-2">{featuredEvent.description}</p>
                    </div>
                    <div className="w-full md:w-1/4 text-center md:text-right">
                        <button 
                            onClick={() => navigateTo('events')} 
                            className="w-full md:w-auto bg-white border-2 border-masa-blue text-masa-blue px-6 py-2.5 rounded-lg font-bold hover:bg-masa-blue hover:text-white transition-all shadow-sm"
                        >
                            View Event Details
                        </button>
                    </div>
        ...
        <section className="py-20 bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                    <div>
                        <h2 className="text-3xl font-bold text-masa-charcoal">Upcoming & Recent Events</h2>
                    ...
                    <button 
                        onClick={() => navigateTo('events')} 
                        className="text-masa-blue font-bold hover:text-masa-orange transition-colors flex items-center group"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {highlightEvents.map((event) => (
                        <div key={event.id} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                            ...
                            <div className="p-6 flex flex-col flex-grow">
                                ...
                                <h3 className="text-lg font-bold text-masa-charcoal mb-3 line-clamp-2">{event.title}</h3>
                                <div className="text-sm text-gray-500 mb-4 space-y-1">
                                    <div className="flex items-center gap-2"><CalendarDaysIcon className="h-4 w-4"/> {event.displayDate}</div>
                                ...
                                <p className="sr-only">
                                    MASA World Foundation Event: {event.title} – A {event.category.toLowerCase()} initiative focused on youth empowerment, education, sports, and social impact in {event.location}.
                                </p>
    ...
    return (
        <section className="py-20 bg-gray-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                    <div>
                        <h2 className="text-3xl font-bold text-masa-charcoal">Latest Blogs & Updates</h2>
                        <p className="mt-2 text-gray-600 max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {latestPosts.map((post) => (
                        <div key={post.id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 group flex flex-col h-full cursor-pointer" onClick={() => navigateTo('blog')}>
                            ...
                            <p className="text-gray-600 text-sm mb-4 line-clamp-3 flex-grow">{post.summary}</p>
...
// --- NEW: PLEDGE HIGHLIGHT SECTION ---
const PledgeHighlightSection: React.FC<NavigationProps> = ({ navigateTo }) => {
    const featuredPledge = pledgeData[0]; // Rotate this or randomize
    const [stats, setStats] = useState({ pledges: 125000 });

    useEffect(() => {
        const s = getStats();
        setStats(prev => ({ ...prev, pledges: 125000 + (s.pledges || 0) }));
    }, []);

    return (
        <section className="bg-masa-charcoal text-white py-16">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-sm border border-white/10 relative overflow-hidden">
                    <div className="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-masa-orange/20 rounded-full blur-3xl"></div>
                    
                    <div className="grid md:grid-cols-2 gap-12 items-center relative z-10">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-masa-orange text-white text-xs font-bold mb-4 uppercase tracking-wider">
                                <HandRaisedIcon className="h-3 w-3" /> Public Pledges for Change
                            </div>
                            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">Join {stats.pledges.toLocaleString()}+ Citizens Taking a Stand</h2>
                            <p className="text-gray-300 mb-8 text-lg">
                                Make a commitment to social responsibility. Take the <strong>{featuredPledge.title}</strong> today and receive your digital certificate instantly.
                            </p>
                            <div className="flex flex-wrap gap-4">
                                <button 
                                    onClick={() => navigateTo('pledge-platform')} 
                        ...
                        <div className="hidden md:flex justify-center">
                            <div className="bg-white p-4 rounded-xl shadow-2xl rotate-3 max-w-xs text-masa-charcoal text-center">
                                <div className="border-4 border-masa-blue p-4 rounded-lg">
                                    <div className="w-16 h-16 mx-auto bg-orange-100 rounded-full flex items-center justify-center mb-4">
                                        <UsersIcon className="h-8 w-8 text-masa-orange" />
                                    </div>
                                    <h3 className="font-bold text-lg mb-1">{featuredPledge.title}</h3>
                                    <p className="text-xs text-gray-500 italic">"I pledge to be a positive role model..."</p>
                                    <div className="mt-4 pt-4 border-t border-gray-100 text-xs font-bold text-masa-blue">
        ...
        <section className="py-24 bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    <h2 className="text-3xl font-bold text-masa-charcoal">Why Masa World Foundation?</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {reasons.map((reason, index) => (
                        <div key={index} className="flex flex-col items-center text-center p-8 bg-gray-50 rounded-2xl border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group">
                            <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mb-6 shadow-md text-masa-blue group-hover:bg-masa-orange group-hover:text-white transition-colors">
                                <reason.icon className="h-9 w-9" />
                            </div>
                            <h3 className="text-xl font-bold text-masa-charcoal mb-4">{reason.title}</h3>
        ...
        <section className="bg-masa-charcoal pt-24 pb-24 text-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto">
                    <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight">Let’s Create Incredible!</h2>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mt-16 max-w-6xl mx-auto">
                    {programs.map(p => (
                        <div key={p.title} className="text-center group cursor-pointer bg-blue-900/20 hover:bg-blue-900/40 p-6 rounded-2xl transition-all transform hover:-translate-y-2" onClick={() => navigateTo(p.page as any)}>
                            <div className={`w-20 h-20 mx-auto ${p.color} rounded-full flex items-center justify-center mb-5 shadow-lg`}>
...
const PillarsSection: React.FC<NavigationProps> = ({ navigateTo }) => {
    ...
    return (
        <section className="py-24 bg-gray-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16"><span className="text-masa-blue font-bold uppercase tracking-widest">Our Pillars</span><h2 className="text-3xl font-bold text-masa-charcoal mt-2">Holistic Development Ecosystem</h2><p className="mt-4 text-gray-600 max-w-3xl mx-auto">We don’t focus on just one aspect. We build a connected ecosystem where sports, education, and culture reinforce each other.</p></div>
                <div className="relative grid md:grid-cols-3 gap-8">
                    ...
                    {pillars.map(pillar => (
                        <div key={pillar.title} className="relative flex flex-col text-center items-center">
                            <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center border-4 border-masa-blue shadow-lg z-10">
                                <pillar.icon className="h-10 w-10 text-masa-blue" />
                            </div>
                            <p className="mt-3 text-sm font-bold text-gray-500 uppercase tracking-widest">{pillar.subtitle}</p>
                            <h3 className="text-2xl font-bold mt-1 text-masa-charcoal">{pillar.title}</h3>
                            <p className="text-gray-600 my-4 flex-grow max-w-xs">{pillar.desc}</p>
                            <button onClick={() => navigateTo(pillar.page as any)} className="mt-auto font-bold text-masa-blue hover:text-masa-orange transition-colors flex items-center group">
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                    {initiatives.map((item) => (
                        <div key={item.title} className="bg-gray-50 p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all border border-gray-100 flex flex-col group">
                            <div className="w-16 h-16 bg-masa-blue text-white rounded-full flex items-center justify-center mb-6 shadow-md group-hover:bg-masa-orange transition-colors">
                            ...
                            <button 
                                ...
                                className="mt-auto font-bold text-masa-blue hover:text-masa-orange transition-colors flex items-center"
                            >
                ...
                <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
                    ...
                    <div className="flex justify-center md:justify-end order-1">
                        <div className="relative">
                            <div className="absolute inset-0 bg-masa-orange/10 transform translate-x-3 translate-y-3 rounded-xl -z-10"></div>
                            <img 
                                src={FOUNDER_IMAGE_URL} 
                                alt="Vijay Babu Sharma" 
                                className="rounded-xl shadow-lg w-full max-w-sm h-auto object-cover aspect-[4/5] border border-gray-100"
                            />
                        </div>
                    </div>
                    ...
                    <div className="max-w-prose mx-auto md:mx-0 order-2 text-left">
                        ...
                        <blockquote className="text-lg text-gray-700 leading-relaxed italic border-l-4 border-masa-orange pl-6 mb-6">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pt-6 border-t border-gray-100">
                            ...
                            <button 
                                ...
                                className="inline-flex items-center text-white bg-masa-blue px-6 py-3 rounded-full font-semibold hover:bg-blue-900 transition-all shadow-md transform hover:-translate-y-0.5"
                            >
                ...
                <div className="border-t border-gray-200 py-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8 text-center">
                        {trustPoints.map((point) => (
                            <div key={point.text} className="flex items-center justify-center text-sm font-medium text-gray-700">
                                <point.icon className="h-5 w-5 mr-3 text-masa-blue flex-shrink-0" />
...
const AccountabilityInfographic: React.FC = () => {
    ...
    return (
        <section className="py-24 bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16"><span className="text-masa-blue font-bold uppercase tracking-widest">Our Accountability</span><h2 className="text-3xl font-bold text-masa-charcoal mt-2">The Cycle of Trust</h2><p className="mt-4 text-lg text-gray-600">Transparency isn’t just a promise; it’s built into every step of our process.</p></div>
    ...
    return (
        <section className="py-24 bg-gray-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="relative max-w-3xl mx-auto min-h-[20rem] flex items-center justify-center">
                    {testimonials.map((testimonial, index) => (
                        <div
                            key={index}
                            className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${index === current ? 'opacity-100' : 'opacity-0'}`}
                            style={{ transitionDelay: index === current ? '0.2s' : '0s' }}
                        >
                            <div className="flex flex-col items-center text-center p-4">
                                ...
                                <blockquote className="text-xl italic text-gray-700 leading-relaxed max-w-2xl">
                                    "{testimonial.quote}"
                                </blockquote>
                                <div className="mt-6">
                    ...
                    <button onClick={prev} aria-label="Previous testimonial" className="absolute top-1/2 -left-4 md:-left-16 transform -translate-y-1/2 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-masa-orange z-10">
                        <ChevronLeftIcon className="h-6 w-6 text-masa-charcoal" />
                    </button>
                    <button onClick={next} aria-label="Next testimonial" className="absolute top-1/2 -right-4 md:-right-16 transform -translate-y-1/2 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-masa-orange z-10">
...
const FinalCTA: React.FC<NavigationProps> = ({ navigateTo }) => (
    <section className="bg-masa-blue"><div className="container mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center"><h2 className="text-4xl font-bold text-white">Be the Change You Want to See</h2><div className="mt-10 flex flex-col sm:flex-row justify-center items-center gap-4"><button onClick={() => navigateTo('volunteer')} className="bg-masa-orange text-white px-8 py-3 rounded-full text-lg font-bold hover:bg-orange-600">Volunteer</button><button onClick={() => navigateTo('membership')} className="bg-white text-masa-charcoal px-8 py-3 rounded-full text-lg font-bold hover:bg-gray-100">Become a Member</button><button onClick={() => navigateTo('donate')} className="bg-transparent border-2 border-white text-white px-8 py-3 rounded-full text-lg font-bold hover:bg-white hover:text-masa-blue">Donate Now</button></div></div></section>
);
...
const HomePage: React.FC<NavigationProps> = ({ navigateTo }) => {
    return (
        <>
            ...
            <ImpactSnapshot />
            <NgoHelpDeskTeaser navigateTo={navigateTo} />
            ...
            <PledgeHighlightSection navigateTo={navigateTo} />
            <WhyMasaSection />
